...
const reglas = () => p.evaluate(() => [...document.querySelectorAll('.rulec')].map(c => c.querySelector('.rname').textContent.trim() + ' [' + c.querySelector('.rst').textContent.trim() + '] ' + (c.querySelector('.rv') ? c.querySelector('.rv').textContent.trim() : '—') + ' · ' + c.querySelector('.rnote').textContent.trim()).join('\n     '));
const est = () => p.evaluate(() => { const c = document.querySelector('.acct'); return c.querySelector('.acc-st .tag').textContent.trim() + ' — ' + c.querySelector('.acc-st .why').textContent.trim().slice(0, 95); });
const vigila = () => p.evaluate(() => [...document.querySelectorAll('.acct .acc-watch .wch')].slice(0,4).map(x => x.textContent.replace(/\s+/g, ' ').trim()).join(' | '));
...
say('indicador:', await p.textContent('#rulesState'));
say('cuenta:', await est()); say('vigilando:', await vigila());
say('regla 03:', await p.evaluate(() => { const c = [...document.querySelectorAll('.rulec')].find(x => /seguidas/i.test(x.querySelector('.rname').textContent)); return c.querySelector('.rst').textContent.trim() + ' · ' + c.querySelector('.rnote').textContent.trim(); }));

console.log('=== instrumento prohibido + exceso de contratos ===');
await op('11:00', 'MGC', 5, 21050);
say('regla 01:', await p.evaluate(() => { const c = document.querySelectorAll('.rulec')[0]; return c.querySelector('.rst').textContent.trim() + ' · ' + c.querySelector('.rnote').textContent.trim(); }));
say('regla 05:', await p.evaluate(() => { const c = [...document.querySelectorAll('.rulec')].find(x => /Contratos/i.test(x.querySelector('.rname').textContent)); return c.querySelector('.rst').textContent.trim() + ' · ' + c.querySelector('.rnote').textContent.trim(); }));
say('indicador:', await p.textContent('#rulesState'));
say('vigilando:', await vigila());
...
say('tras archivar:', await p.evaluate(() => { const c = document.querySelectorAll('.acct')[1]; return c.querySelector('.acc-state').textContent.trim() + ' · ' + c.querySelector('.acc-st .tag').textContent.trim(); }));
say('meta cuentas:', (await p.textContent('#acctsMeta')).replace(/\s+/g, ' '));
